...
export type Save={version:3;day:number;phase:Phase;minutes:number;coins:number;materials:Record<MaterialId,number>;inventory:Record<ProductId,number>;hired:WorkerId[];placedBenches:Record<WorkerId,number|null>;workerState:Record<WorkerId,WorkerState>;customers:Customer[];nextCustomer:number;served:number;sales:number;revenue:number;expenses:number;reputation:number;display:ProductId[];storageLevel:number;tutorial:number;paused:boolean}
...
export const freshSave=():Save=>({version:3,day:1,phase:'prep',minutes:8*60,coins:150,materials:{iron:8,leather:8,herbs:8,wood:8},inventory:emptyInventory(),hired:['blacksmith','alchemist'],placedBenches:{blacksmith:0,leatherworker:null,alchemist:5,carpenter:null},workerState:workerState(),customers:[],nextCustomer:1,served:0,sales:0,revenue:0,expenses:0,reputation:0,display:['shortsword','healingPotion'],storageLevel:1,tutorial:0,paused:false})
export const loadSave=():Save=>{try{const s=JSON.parse(localStorage.getItem('magic-and-steel-save')||'null');if(s?.version===3)return s;if(s?.version===2)return{...s,version:3,placedBenches:{blacksmith:s.hired.includes('blacksmith')?0:null,leatherworker:s.hired.includes('leatherworker')?2:null,alchemist:s.hired.includes('alchemist')?5:null,carpenter:s.hired.includes('carpenter')?7:null}};return freshSave()}catch{return freshSave()}}
