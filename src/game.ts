...
export type Save={version:14;day:number;phase:Phase;minutes:number;coins:number;materials:Record<MaterialId,number>;inventory:Record<ProductId,number>;hired:WorkerId[];placedBenches:Record<WorkerId,number|null>;shopkeeperSlot:number|null;displays:Record<DisplayId,DisplayState>;ownedDisplays:Record<DisplayId,boolean>;furnitureFacing:Record<FurnitureId,FurnitureFacing>;workerState:Record<WorkerId,WorkerState>;customers:Customer[];discoveredCustomers:CustomerSprite[];nextCustomer:number;served:number;sales:number;revenue:number;expenses:number;reputation:number;storageLevel:number;tutorial:number;paused:boolean;speed:1|2;autoPause:boolean}
...
export const freshSave=():Save=>({version:14,day:1,phase:'prep',minutes:8*60,coins:150,materials:{iron:8,leather:8,herbs:8,wood:8},inventory:emptyInventory(),hired:['blacksmith','alchemist'],placedBenches:{blacksmith:6,leatherworker:null,alchemist:23,carpenter:null},shopkeeperSlot:16,displays:{shelf:{slot:12,product:null,quantity:0},weaponRack:{slot:null,product:null,quantity:0},armourStand:{slot:null,product:null,quantity:0},potionDisplay:{slot:null,product:null,quantity:0}},ownedDisplays:{shelf:true,weaponRack:false,armourStand:false,potionDisplay:false},furnitureFacing:{blacksmith:0,leatherworker:0,alchemist:0,carpenter:0,shopkeeper:0,shelf:0,weaponRack:0,armourStand:0,potionDisplay:0},workerState:workerState(),customers:[],discoveredCustomers:[],nextCustomer:1,served:0,sales:0,revenue:0,expenses:0,reputation:0,storageLevel:1,tutorial:0,paused:false,speed:1,autoPause:true})
export const loadSave=():Save=>{try{
 ...
 return{...s,version:14,displays,discoveredCustomers,inventory:{...emptyInventory(),...s.inventory},speed:s.speed===2?2:1,autoPause:s.autoPause!==false}
