...
export type Save={version:5;day:number;phase:Phase;minutes:number;coins:number;materials:Record<MaterialId,number>;inventory:Record<ProductId,number>;hired:WorkerId[];placedBenches:Record<WorkerId,number|null>;shopkeeperSlot:number|null;workerState:Record<WorkerId,WorkerState>;customers:Customer[];nextCustomer:number;served:number;sales:number;revenue:number;expenses:number;reputation:number;display:ProductId[];storageLevel:number;tutorial:number;paused:boolean}
...
export const freshSave=():Save=>({version:5,day:1,phase:'prep',minutes:8*60,coins:150,materials:{iron:8,leather:8,herbs:8,wood:8},inventory:emptyInventory(),hired:['blacksmith','alchemist'],placedBenches:{blacksmith:6,leatherworker:null,alchemist:23,carpenter:null},shopkeeperSlot:16,workerState:workerState(),customers:[],nextCustomer:1,served:0,sales:0,revenue:0,expenses:0,reputation:0,display:['shortsword','healingPotion'],storageLevel:1,tutorial:0,paused:false})
export const loadSave=():Save=>{try{const s=JSON.parse(localStorage.getItem('magic-and-steel-save')||'null');if(s?.version===5)return s;if(s?.version===4)return{...s,version:5,shopkeeperSlot:16};if(s?.version===3){const map=[6,8,10,16,19,21,23,17];return{...s,version:5,shopkeeperSlot:16,placedBenches:Object.fromEntries(Object.entries(s.placedBenches).map(([id,slot])=>[id,slot===null?null:map[Number(slot)]??null])) as Save['placedBenches']}}if(s?.version===2)return{...s,version:5,shopkeeperSlot:16,placedBenches:{blacksmith:s.hired.includes('blacksmith')?6:null,leatherworker:s.hired.includes('leatherworker')?10:null,alchemist:s.hired.includes('alchemist')?23:null,carpenter:s.hired.includes('carpenter')?17:null}};return freshSave()}catch{return freshSave()}}
