...
export type Save={version:2;day:number;phase:Phase;minutes:number;coins:number;materials:Record<MaterialId,number>;inventory:Record<ProductId,number>;hired:WorkerId[];workerState:Record<WorkerId,WorkerState>;customers:Customer[];nextCustomer:number;served:number;sales:number;revenue:number;expenses:number;reputation:number;display:ProductId[];storageLevel:number;tutorial:number;paused:boolean}
...
export const freshSave=():Save=>({version:2,day:1,phase:'prep',minutes:8*60,coins:150,materials:{iron:8,leather:8,herbs:8,wood:8},inventory:emptyInventory(),hired:['blacksmith','alchemist'],workerState:workerState(),customers:[],nextCustomer:1,served:0,sales:0,revenue:0,expenses:0,reputation:0,display:['shortsword','healingPotion'],storageLevel:1,tutorial:0,paused:false})
export const loadSave=():Save=>{try{const s=JSON.parse(localStorage.getItem('magic-and-steel-save')||'null');return s?.version===2?s:freshSave()}catch{return freshSave()}}
