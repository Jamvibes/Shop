...
import {
  clock, craftableProducts, craftTicks, customerTemplates, freshSave, loadSave, makeCustomer, offerFit, priceFor, products, recipesFor,
...
function App() {
  ...
  function tick(s: Save) {
    ...
    if (gone.length) { s.reputation = Math.max(-2, s.reputation - gone.length); s.missedRequests+=gone.length; setNote(`${gone[0].name} leaves after waiting.`) }
  ...
  function closeAccounts(s: Save) {
    ...
    s.wagesPaid=wages;s.deliveryPaid=delivery
  ...
  const serve = (action: 'sell' | 'haggle' | 'suggest' | 'wait' | 'refuse' | 'buy') => update(s => {
    ...
    if (action === 'refuse') { s.customers.shift(); s.served++; s.missedRequests++; setNote(`${c.name} leaves.`); return s }
    ...
    const r = products[selected],fit=offerFit(c,selected,action==='suggest')
    if(['tooHigh','tooLow','wrongItem','wrongCategory'].includes(fit)){
      const reason=fit==='wrongItem'?`“I specifically asked for ${products[c.requestedProduct!].name}.”`:fit==='wrongCategory'?`“That isn't the kind of item I need.”`:fit==='tooLow'?`“I need something better than this for the road ahead.”`:['warden','ranger','knight','paladin'].includes(c.sprite)?`“I'm not seasoned enough to use equipment of that tier.”`:`“That is beyond what I can afford right now.”`
      if(action==='suggest')s.reputation--
      s.missedRequests++;s.customers.shift();s.served++;setSelected(null);setNote(`${c.name}: ${reason}`);return s
    }
    const displayBoost = stockedDisplay ? .1 : 0
    const base = Math.round(priceFor(c, selected, s.reputation) * (stockedDisplay ? 1.08 : 1)), success = action === 'sell' ? true : action === 'suggest' ? Math.random()<Math.min(.9,.68+displayBoost) : Math.random() < Math.min(.95, (fit==='exact'?.84:.72) + displayBoost)
    if (success) { const price = action === 'haggle' ? Math.round(base * 1.18) : base; if(stockedDisplay){const display=s.displays[stockedDisplay];display.quantity--;s.displaySales++;if(display.quantity<=0){display.product=null;display.quantity=0}}else s.inventory[selected]--; s.coins += price; s.revenue += price; s.sales++; s.dailyProductSales[selected]++; setDeparture({sprite:c.sprite,product:selected}); setTimeout(()=>setDeparture(null),1800); setNote(`${c.name} buys ${r.name} for ${price}g${stockedDisplay?' after spotting it on display':''}.`) }
    else { s.missedRequests++;if(action === 'haggle')s.reputation--; setNote(action === 'haggle' ? `${c.name} rejects the negotiation.` : `${c.name} considers the alternative, but decides against ${r.name}.`) }
    s.customers.shift(); s.served++; setSelected(null); return s
  })
  const nextDay = () => update(s => { s.day++; s.phase = 'prep'; s.minutes = 480; s.customers = []; s.served = 0; s.sales = 0; s.dailyProductSales=Object.fromEntries(productIds.map(id=>[id,0])) as Record<ProductId,number>;s.missedRequests=0;s.displaySales=0;s.revenue = 0; s.expenses = 0;s.wagesPaid=0;s.deliveryPaid=0;s.reputationStart=s.reputation; setNote(`Day ${s.day}. Arrange the shop before opening.`); return s })
  ...
  const bestSeller = productIds.reduce<ProductId | null>((best,id)=>g.dailyProductSales[id]>(best?g.dailyProductSales[best]:0)?id:best,null)
  const netProfit=g.revenue-g.expenses
  const reputationChange=g.reputation-g.reputationStart
  ...
  return <div className="game-shell">
    ...
    <main><section className={`scene ${placing ? 'placement-mode' : ''}`}><div className="sign">✦ THE WANDERING ANVIL ✦</div><div className="room">
      <div className="shop-ground" aria-hidden="true"/>
      <div className="road-entry-depth" aria-hidden="true"/><div className="road-entry" aria-hidden="true"/>
      <div className="front-path-depth" aria-hidden="true"/><div className="front-path" aria-hidden="true"/>
      <div className="floor-depth" aria-hidden="true"/>
      <div className="wall-face wall-face-left" aria-hidden="true"/><div className="wall-face wall-face-right" aria-hidden="true"/>
      <div className="wall-top wall-top-left" aria-hidden="true"/><div className="wall-top wall-top-right" aria-hidden="true"/><div className="wall-corner-cap" aria-hidden="true"/>
    ...
    <aside className="ledger"><div className="ledger-head"><b>SHOP LEDGER</b><button onClick={reset}>{resetArmed ? 'CONFIRM RESET' : 'RESET SAVE'}</button></div>
      ...
      {g.phase === 'open' && <><div className="open-controls"><button onClick={() => update(s => { s.paused = !s.paused; return s })}>{g.paused ? '▶ RESUME' : 'Ⅱ PAUSE'}</button><div className="speed-controls" aria-label="Game speed"><button className={g.speed === 1 ? 'active' : ''} onClick={() => update(s => { s.speed = 1; return s })}>1×</button><button className={g.speed === 2 ? 'active' : ''} onClick={() => update(s => { s.speed = 2; return s })}>2×</button></div><span>Visitors {g.customers.length}/3</span></div>{active ? <section className="trade"><h2>{active.icon} {active.name}</h2><small className="role">Customer level {active.level} · accepts product tiers {Math.max(1,active.level-1)}–{active.level}</small><p>{active.kind === 'supplier' ? `“I can sell you ${active.amount} ${active.material} for ${active.cost}g.”` : active.requestedProduct ? `“I'm looking for a ${products[active.requestedProduct].name}. Do you have one?”` : `“I'm looking for something in the ${active.request} category. What can you offer?”`}</p>{selected && active.kind === 'buyer' && <div className="quoted">{products[selected].icon} {products[selected].name} · Tier {products[selected].level} · {offer}g</div>}<div className="trade-actions">{active.kind === 'supplier' ? <><button onClick={() => serve('buy')}>ACCEPT OFFER</button><button onClick={() => serve('refuse')}>DECLINE</button></> : <><button disabled={!selected} onClick={() => serve('sell')}>SELL ITEM</button><button disabled={!selected} onClick={() => serve('haggle')}>NEGOTIATE +18%</button><button disabled={!selected} onClick={() => serve('suggest')}>SUGGEST ALTERNATIVE</button><button onClick={() => serve('wait')}>ASK TO WAIT</button><button onClick={() => serve('refuse')}>REFUSE SALE</button></>}</div></section> : <p className="empty">The counter is clear.</p>}</>}
      {g.phase === 'results' && <section className="results day-report"><h2>Day {g.day} report</h2><div className={`report-hero ${netProfit>=0?'profit':'loss'}`}><small>NET RESULT</small><b>{netProfit>=0?'+':''}{netProfit}g</b><span>{netProfit>=0?'A profitable day at the Wandering Anvil.':'Tomorrow offers another chance.'}</span></div><div className="report-grid"><article><span>Items sold</span><b>{g.sales}</b></article><article><span>Missed requests</span><b>{g.missedRequests}</b></article><article><span>Display-assisted</span><b>{g.displaySales}</b></article><article><span>Reputation</span><b>{reputationChange>=0?'+':''}{reputationChange}</b></article></div><div><span>Sales revenue</span><b>+{g.revenue}g</b></div><div><span>Worker wages</span><b>-{g.wagesPaid}g</b></div><div><span>Material deliveries</span><b>-{g.deliveryPaid}g</b></div><div className="best-seller"><span>Best seller</span><b>{bestSeller?<><ItemSprite product={bestSeller}/>{products[bestSeller].name} ×{g.dailyProductSales[bestSeller]}</>:'No sales recorded'}</b></div><button className="upgrade" onClick={() => update(s => { if (s.coins >= 65) { s.coins -= 65; s.storageLevel++; setNote('Stockroom expanded.') } return s })}>EXPAND STOCKROOM · 65g<small>+4 material capacity</small></button><button className="primary" onClick={nextDay}>BEGIN DAY {g.day + 1}</button></section>}
    </aside></main>
