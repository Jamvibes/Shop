...
function App() {
  ...
  const serve = (action: 'sell' | 'haggle' | 'suggest' | 'wait' | 'refuse' | 'buy') => update(s => {
    ...
    if (action === 'suggest' && !liked) { s.reputation--; s.customers.shift(); s.served++; s.missedRequests++; setNote(`${c.name} dislikes the suggestion.`); return s }
    ...
    else { s.missedRequests++;if(action === 'haggle')s.reputation--; setNote(action === 'haggle' ? `${c.name} rejects the negotiation.` : `${c.name} refuses ${r.name}; it is not what they requested.`) }
