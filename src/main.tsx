...
function App() {
  ...
  const storeFurniture = (id: PlacementId) => update(s => {
    ...
    } else if(id==='shopkeeper') { s.shopkeeperSlot=null; setActiveFixture(null) }
    ...
    <main><section className={`scene ${placing ? 'placement-mode' : ''}`}><div className="sign">✦ THE WANDERING ANVIL ✦</div><div className="room">
      ...
      <div className="workshops">{workerIds.map(id => {
        ...
        return <div className={`placed-workstation facing-${g.furnitureFacing[id]} ${worker === id ? 'chosen' : ''}`} style={{ left: `${x}%`, top: `${y}%`, '--coat': w.color } as React.CSSProperties} key={id}>
          <button className="map-bench" title={`${benchNames[id]} furniture controls`} onClick={()=>setActiveFixture(id)} aria-label={`Open ${benchNames[id]} furniture controls`}><span>{w.icon}</span></button>
      ...
      {g.shopkeeperSlot !== null && (() => { const [x, y] = slotPositions[g.shopkeeperSlot]; return <div className={`shopkeeper-station facing-${g.furnitureFacing.shopkeeper}`} style={{ left: `${x}%`, top: `${y}%` }}><button className="shop-counter" onClick={()=>setActiveFixture('shopkeeper')} aria-label="Open Sales Counter furniture controls"><span>▤</span></button><img src={`${import.meta.env.BASE_URL}assets/shop/shopkeeper.png`} alt="Shopkeeper" draggable="false"/><small>SHOPKEEPER</small></div> })()}
      {activeFixture && (()=>{const slot=activeFixture==='shopkeeper'?g.shopkeeperSlot:g.placedBenches[activeFixture];if(slot===null)return null;const [x,y]=slotPositions[slot];return <div className="display-popup fixture-popup" style={{left:`${x}%`,top:`${y}%`}} role="dialog" aria-label={`${benchNames[activeFixture]} controls`}><button className="picker-close" onClick={()=>setActiveFixture(null)}>×</button><b>{benchNames[activeFixture]}</b><small>{activeFixture==='shopkeeper'?'The shop cannot open without its counter.':'Storing the bench sends its worker home until it is placed again.'}</small><button disabled={g.phase!=='prep'} onClick={()=>rotateFurniture(activeFixture)}>ROTATE ↻</button><button disabled={g.phase!=='prep'} onClick={()=>moveFixture(activeFixture)}>MOVE</button><button disabled={g.phase!=='prep'} onClick={()=>storeFurniture(activeFixture)}>PLACE IN STORAGE</button></div>})()}
